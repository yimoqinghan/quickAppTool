  ...
  <el-container>
    ...
    <el-main>
      ...
      <div v-if="selectedAppInfo" class="app-details-card">
        ...
        <p v-if="selectedAppInfo.company">公司: {{ selectedAppInfo.company }}</p>
      ...
      <div v-if="!selectedAppInfo">
        ...
        <div v-else class="app-card-list">
          <div
            ...
            :key="app.packageName"
            class="app-card"
            @click="getAppDetails(app)"
          >
            <img :src="app.iconUrl" alt="App Icon" class="app-icon" />
            <div class="app-info">
              <h3>{{ app.name }}</h3>
              <p>版本号: {{ app.vName }}</p>
              <p>包名: {{ app.pkgName }}</p>
  ...
  setup() {
    ...
    const searchApp = async () => {
      try {
        ...
        const response = await axios.get('/opv/jitscenter/center/instants/search', {
          params: {
            name: `%${appName.value}%`,
            size: pageSize.value,
        ...
        if (response.data.code === '200') {
          appList.value = response.data.data.instantDto;
        } else {
          appList.value = [];
          alert('未找到匹配的应用');
        }
    ...
    const getAppDetails = async (appInfo) => {
      try {
        const details = {
          link: appInfo.url,
